    ...
    public void visit( UserFuncExpression op ) throws FrontendException {       
        ...
        if (f instanceof EvalFunc) {
            ...
            if( op.getFuncSpec().toString().equals(ROLLUP_UDF))
            {
                ...
                if(op.getRollupH2IRGOptimizable()!=false) {
                    ((POUserFunc)p).setRollupH2IRGOptimizable(op.getRollupH2IRGOptimizable());
                    ((POUserFunc)p).setRollupFieldIndex(op.getRollupFieldIndex());
                    ((POUserFunc)p).setRollupOldFieldIndex(op.getRollupOldFieldIndex());
                    ((POUserFunc)p).setDimensionSize(op.getDimensionSize());
                    ((POUserFunc)p).setChunkSize(op.getChunkSize());
                    ((POUserFunc)p).setRate(op.getRate());
                    EvalFunc<?> tmp = ((POUserFunc)p).getFunc();
                    ((RollupDimensions)tmp).setRollupH2IRGOptimizable(true);
                    try {
