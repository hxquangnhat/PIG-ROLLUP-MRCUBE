            ...
            while (it.hasNext()) {
                ...
                if (op instanceof LOGenerate) {
                    ...
                    for (LogicalExpressionPlan op2 : inner_leplan) {
                        for (Operator op3 : op2.getSources()) {
                            if (op3 instanceof UserFuncExpression) {
                                ...
                                dimensionSize += uf.getFieldSchema().schema.getFields().get(0).schema.getFields().size();
                                ...
                                if (uf.getFuncSpec().toString().equals(ROLLUP_UDF)) {
                                    ...
                                    if (nRollup == 0) {
                                        //the original position of the first field of the rollup operation
                                        rollupOldFieldIndex = dimensionSize - uf.getFieldSchema().schema.getFields().get(0).schema.getFields().size();
                                        ...
                                        for (LogicalFieldSchema a : foreach_field_lst)
                                            if (a.alias.equals(first_rollup.alias)) {
                                                rollupUFIndex = inner_leplan.indexOf(op2);
                                                break;
                                            }
                                        
                                        rollupSize = uf.getFieldSchema().schema.getFields().get(0).schema.getFields().size();
                                        
                                        //if(cogroup.getPivot()!=-1)
                                            uf.setPivot(cogroup.getPivot());
                                        //else 
                                            //uf.setPivot(0);
                                        uf.setRollupH2IRGOptimizable(true);
                                        uf.setRollupFieldIndex(dimensionSize - rollupSize);
                                        uf.setRollupOldFieldIndex(rollupOldFieldIndex);
                                        //set field start
                                        //set position
                                    }
                                }
                    ...
                    for (LogicalExpressionPlan op2 : inner_leplan)
                        for (Operator op3 : op2.getSources()) {
                            if (op3 instanceof UserFuncExpression) {
                                ...
                                if (uf.getFuncSpec().toString().equals(ROLLUP_UDF)) {
                                    uf.setRollupFieldIndex(dimensionSize - rollupSize);
                                    uf.setDimensionSize(dimensionSize);
                                    //if(cogroup.getPivot() == -1) {
                                        uf.setChunkSize(cogroup.getChunkSize());
                                        uf.setRate(cogroup.getRate());
                                    //}
                                }
                            }
                        }
                    ...
                    if (rollupUFIndex < ufSize -1) {
                        LogicalExpressionPlan temp = inner_leplan.get(rollupUFIndex);
                    
                        for (int l = rollupUFIndex; l < ufSize - 1; l++)
                            inner_leplan.set(l, inner_leplan.get(l+1));
                    
                        inner_leplan.set(ufSize - 1, temp);
                    }
