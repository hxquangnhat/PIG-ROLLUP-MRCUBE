...
public class POLocalRearrange extends PhysicalOperator {
    ...
    public Result getNextTuple() throws ExecException {
        ...
        while (true) {
            ...
            // if mrcube
            // res.result = constructLROutput(secondaryResLst, resLst, (Tuple)inp.result);
            // secondaryResLst.add(count)
            if (isSample) {
                Tuple tmpRes = (Tuple)inp.result;
                Tuple tmpKey = (Tuple) tmpRes.get(0);
                int count = 0;
                for (int i = 0; i < tmpKey.size(); i++)
                    if (tmpKey.get(i) == null)
                        count++;
                Result region = new Result();
                region.result = count;
                secondaryResLst.add(region);
