...
public class XMLPhysicalPlanPrinter<P extends OperatorPlan<PhysicalOperator>>
    ...
    public XMLPhysicalPlanPrinter(PhysicalPlan plan, Document doc,
            Element parent) {
    ...
    private void visitPlan(PhysicalPlan pp, Element parentNode)
            throws VisitorException {
        if (pp != null) {
            XMLPhysicalPlanPrinter<PhysicalPlan> ppp = new XMLPhysicalPlanPrinter<PhysicalPlan>(
                    pp, doc, parentNode);
            ppp.visit();
        }
    }

    private void visitPlan(List<PhysicalPlan> lep, Element parentNode)
            throws VisitorException {
        if (lep != null)
    ...
    private Element createPONode(PhysicalOperator node) {
        ...
        if (node instanceof POStore) {
            ...
            isTmpStore.setTextContent(Boolean.valueOf(
                    ((POStore) node).isTmpStore()).toString());
        ...
        if (node instanceof POLoad) {
            ...
            isTmpLoad.setTextContent(Boolean.valueOf(
                    ((POLoad) node).isTmpLoad()).toString());
            PONode.appendChild(isTmpLoad);
        }
        return PONode;
    }

    private void depthFirst(PhysicalOperator node, Element parentNode)
            throws VisitorException {
        ...
        if (node instanceof POFilter) {
            ...
            subPlans = ((PORollupH2IRGForEach) node).getInputPlans();
            ...
            MultiMap<PhysicalOperator, PhysicalPlan> joinPlans = skewed
                    .getJoinPlans();
            if (joinPlans != null) {
        ...
        List<PhysicalOperator> originalPredecessors = mPlan
                .getPredecessors(node);
        if (originalPredecessors == null) {
            return;
        }

        List<PhysicalOperator> predecessors = new ArrayList<PhysicalOperator>(
