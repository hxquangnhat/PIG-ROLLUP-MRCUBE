...
public class PORollupSampling extends POForEach {
    ...
    private final String allMarker = null;
    
    private List<Tuple> result = null; 

    public static long tPOSampling = 0;
    
    public static long tCachePOSampling = 0;

    public long sTime = 0;

    
    public PORollupSampling(OperatorKey k) {
        this(k,-1,null,null);
    }

    public PORollupSampling(OperatorKey k, int rp) {
        this(k,rp,null,null);
    }

    public PORollupSampling(OperatorKey k, List inp) {
        this(k,-1,inp,null);
    }

    public PORollupSampling(OperatorKey k, int rp, List<PhysicalPlan> inp, List<Boolean>  isToBeFlattened){
        super(k, rp);
    ...
    public PORollupSampling(OperatorKey operatorKey, int requestedParallelism,
    ...
    public void visit(PhyPlanVisitor v) throws VisitorException {
        v.visitPORollupSampling(this);
    }

    @Override
    public String name() {
        return getAliasString() + "Rollup Sampling" + "(" + getFlatStr() + ")" + "["
    ...
    public Result getNextTuple() throws ExecException {
        try {
            ...
            //System.out.println("POSampling start: " + sTime);
            Result res = null;
            Result inp = null;
            ...
            if(processingPlan){
                while(true) {
                    ...
                    if(res.returnStatus==POStatus.STATUS_OK) {
                        tPOSampling += System.currentTimeMillis() - sTime;
                        //System.out.println("POSampling cache end: " + System.currentTimeMillis());
                        tCachePOSampling += System.currentTimeMillis() - cacheStart;
                        return res;
                    }
            ...
            while (true) {
                ...
                tPOSampling += System.currentTimeMillis() - sTime;
                //System.out.println("POSampling end: " + System.currentTimeMillis());
                processingPlan = true;
                return res;
            }
...
//                Object input = null;
    ...
    public void Vanilla(Tuple tuple) throws IOException {
        result = Lists.newArrayListWithCapacity(tuple.size() + 1);
     ...
     */
    @Override
    public PORollupSampling clone() throws CloneNotSupportedException {
        ...
        PORollupSampling clone = new PORollupSampling(new OperatorKey(mKey.scope,
