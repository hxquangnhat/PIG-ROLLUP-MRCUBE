...
/**
 * FetchOptimizer determines whether the entire physical plan is fetchable,
 * meaning that the task's result can be directly read (fetched) from the
 * underlying storage rather than creating MR jobs. During the check
 * {@link FetchablePlanVisitor} is used to walk through the plan.
 * 
 */
public class FetchOptimizer {
    ...
    public static boolean isFetchEnabled(PigContext pc) {
        return "true".equalsIgnoreCase(pc.getProperties().getProperty(
                PigConfiguration.OPT_FETCH, "true"));
    }
     ...
     */
    public static boolean isPlanFetchable(PigContext pc, PhysicalPlan pp)
            throws VisitorException {
        if (isEligible(pc, pp)) {
    ...
    private static void init(PhysicalPlan pp) throws VisitorException {
        ...
        LinkedList<POStream> posList = PlanHelper.getPhysicalOperators(pp,
                POStream.class);
    ...
    private static boolean isEligible(PigContext pc, PhysicalPlan pp) {
        ...
        for (PhysicalOperator po : roots) {
            if (!(po instanceof POLoad)) {
                String msg = "Expected physical operator at root is POLoad. Found : "
                        + po.getClass().getCanonicalName()
                        + ". Fetch optimizer will be disabled.";
                LOG.debug(msg);
                return false;
            }
        }
        ...
        if (pp.getLeaves().size() != 1) {
            LOG.debug("Expected physical plan should have one leaf. Found "
                    + leafSize);
            return false;
        }
    ...
    private static class FetchablePlanVisitor extends PhyPlanVisitor {
        ...
        public void visitStore(POStore st) throws VisitorException {
            ...
            // Otherwise check if target storage format equals to the
        ...
        @Override
        public void visitRollupCombinerPackage(PORollupCombinerPackage pkg)
                throws VisitorException {
            planFetchable = false;
        }
        ...
        private boolean isTempPath(String basePathName)
                throws DataStorageException {
            String tdir = pc.getProperties()
                    .getProperty("pig.temp.dir", "/tmp");
            String tempStore = pc.getDfs().asContainer(tdir + "/temp")
                    .toString();
            Matcher matcher = Pattern.compile(tempStore + "-?[0-9]+").matcher(
                    basePathName);
