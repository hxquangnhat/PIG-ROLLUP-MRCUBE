...
/**
 * This class is used by skewed join. For the partitioned table, the
 * skewedpartitioner reads the key distribution data from the sampler file and
 * returns the reducer index in a round robin fashion. For ex: if the key
 * distribution file contains (k1, 5, 3) as an entry, reducers from 5 to 3 are
 * returned in a round robin manner.
 */
public class SkewedPartitioner extends
        Partitioner<PigNullableWritable, Writable> implements Configurable {
    ...
    public int getPartition(PigNullableWritable wrappedKey, Writable value,
        ...
        PigNullableWritable key = ((NullablePartitionWritable) wrappedKey)
                .getKey();
        ...
        // if the reducerMap does not contain the key, do the default hash based
        // partitioning
        if (indexes == null) {
