...
/**
 * This class is the base class for PigMapBase, which has slightly difference
 * among different versions of hadoop. PigMapBase implementation is located in
 * $PIG_HOME/shims.
 **/
public abstract class PigGenericMapBase extends
    ...
    /**
     * Will be called when all the tuples in the input are done. So reporter
    ...
    public void cleanup(Context context) throws IOException,
        ...
        if (PigMapReduce.sJobConfInternal.get().get(
                JobControlCompiler.END_OF_INP_IN_MAP, "false").equals("true")) {
        ...
        UDFFinishVisitor finisher = new UDFFinishVisitor(mp,
                new DependencyOrderWalker<PhysicalOperator, PhysicalPlan>(mp));
        try {
            finisher.visit();
    ...
    public void setup(Context context) throws IOException, InterruptedException {
        ...
        PigContext.setPackageImportList((ArrayList<String>) ObjectSerializer
                .deserialize(job.get("udf.import.list")));
        pigContext = (PigContext) ObjectSerializer.deserialize(job
                .get("pig.pigContext"));

        // This attempts to fetch all of the generated code from the distributed
        // cache, and resolve it
        ...
        keyType = ((byte[]) ObjectSerializer.deserialize(job
                .get("pig.map.keytype")))[0];
        // till here
        ...
        if (dtzStr != null && dtzStr.length() > 0) {
            ...
            // style
            DateTimeZone.setDefault(DateTimeZone.forOffsetMillis(DateTimeZone
                    .forID(dtzStr).getOffset(null)));
        }
    }

    /**
     * The map function that attaches the inpTuple appropriately and executes
     * the map plan if its not empty. Collects the result of execution into oc
     * or the input directly to oc if map plan empty. The collection is left
     * abstract for the map-only or map-reduce job to implement. Map-only
     * collects the tuple as-is whereas map-reduce collects it after extracting
     * the key and indexed tuple.
     */
    @Override
    protected void map(Text key, Tuple inpTuple, Context context)
    ...
    abstract public void collect(Context oc, Tuple tuple)
    ...
    abstract public Context getIllustratorContext(Configuration conf,
            DataBag input, List<Pair<PigNullableWritable, Writable>> output,
