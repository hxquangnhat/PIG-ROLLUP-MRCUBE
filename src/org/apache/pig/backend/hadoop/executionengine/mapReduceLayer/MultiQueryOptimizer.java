...
/**
 * An optimizer that merges all or part splittee MapReduceOpers into splitter
 * MapReduceOper.
 * <p>
 * The merge can produce a MROperPlan that has fewer MapReduceOpers than
 * MapReduceOpers in the original MROperPlan.
 * <p>
 * The MRCompler generates multiple MapReduceOpers whenever it encounters a
 * split operator and connects the single splitter MapReduceOper to one or more
 * splittee MapReduceOpers using store/load operators:
 * <p>
 * ---- POStore (in splitter) -... ---- | | ... | | | ... | POLoad POLoad ...
 * POLoad (in splittees) | | |
 * <p>
...
class MultiQueryOptimizer extends MROpPlanVisitor {
    ...
    MultiQueryOptimizer(MROperPlan plan, boolean inIllustrator) {
        super(plan,
                new ReverseDependencyOrderWalker<MapReduceOper, MROperPlan>(
                        plan));
    ...
    public void visitMROp(MapReduceOper mr) throws VisitorException {
        ...
        log.info("Merged " + numMerges + " out of total " + (numSplittees + 1)
                + " MR operators.");
    }
    ...
    private int mergeDiamondMROper(MapReduceOper mr, List<MapReduceOper> succs)
        ...
        for (MapReduceOper succ : succs) {
            ...
            ArrayList<PhysicalOperator> rootsCopy = new ArrayList<PhysicalOperator>(
                    roots);
            for (PhysicalOperator op : rootsCopy) {
                ...
                if (inIllustrator) {
                    // need to remove the LOAD since data from load on temporary
                    // files can't be handled by illustrator
                    for (Iterator<PhysicalOperator> it = pl.iterator(); it
                            .hasNext();) {
    ...
    private void mergeOneMapPart(MapReduceOper mapper, MapReduceOper splitter)
        ...
        List<PhysicalOperator> predsCopy = new ArrayList<PhysicalOperator>(
                storePreds);
    ...
    private List<MapReduceOper> getMergeList(MapReduceOper splitter,
        ...
        if (max == mergeDistList.size())
            return mergeDistList;
        else if (max == mergeNoCmbList.size())
    ...
    private int setIndexOnLRInSplit(int initial, POSplit splitOp,
            boolean sameKeyType) throws VisitorException {
        int index = initial;
    ...
    private void mergeOneReducePlanWithIndex(PhysicalPlan from,
            PhysicalPlan to, int initial, int current, byte mapKeyType)
            throws VisitorException {
        ...
        if (pk instanceof POMultiQueryPackage) {
            ...
            pkg.addIsKeyWrappedList(((POMultiQueryPackage) pk)
                    .getIsKeyWrappedList());
    ...
    private void addShiftedKeyInfoIndex(int index, POPackage pkg)
            throws OptimizerException {
        /**
         * we only do multi query optimization for single input MROpers Hence
         * originally the keyInfo would have had only index 0. As we merge
         * MROpers into parent MROpers we add entries for the multiquery based
         * index (ORed with multi query bit mask). These additions would mean we
         * have many entries in the keyInfo while really it should only have one
         * since there is only one input that the package would be processing
         * and hence only one index. So each time we add an entry for a new
         * shifted index, we should clean up keyInfo so that it has only one
         * entry - the valid entry at that point. The "value" in the keyInfo map
         * for the new addition should be the same as the "value" in the
         * existing Entry. After addition, we should remove the older entry
         */
    ...
    private void mergeAllMapReduceSplittees(List<MapReduceOper> mergeList,
        ...
        PhysicalPlan comPl = needCombiner(mergeList) ? createDemuxPlan(
                sameKeyType, true) : null;
        ...
        for (MapReduceOper mrOp : mergeList) {
            ...
            // > index + 1
            int incIndex = mergeOneMapPlanWithIndex(mrOp.mapPlan, splitOp,
                    index, sameKeyType);

            // merge the combiner plan
            if (comPl != null) {
                if (!mrOp.combinePlan.isEmpty()) {
                    mergeOneCombinePlanWithIndex(mrOp.combinePlan, comPl,
                            index, incIndex, mrOp.mapKeyType);
                } else {
                    int errCode = 2141;
            ...
            mergeOneReducePlanWithIndex(mrOp.reducePlan, redPl, index,
                    incIndex, mrOp.mapKeyType);
        ...
        splitter.mapKeyType = sameKeyType ? mergeList.get(0).mapKeyType
                : DataType.TUPLE;
    ...
    /**
     * Removes the specified MR operator from the plan after the merge. Connects
     * its predecessors and successors to the merged MR operator
     * 
     * @param mr
     *            the MR operator to remove
     * @param newMR
     *            the MR operator to be connected to the predecessors and the
     *            successors of the removed operator
     * @throws VisitorException
     *             if connect operation fails
     */
    ...
    private POMultiQueryPackage getMultiQueryPackage(boolean sameMapKeyType,
            boolean inCombiner) {
        POMultiQueryPackage pkg = new POMultiQueryPackage(new OperatorKey(
