...
chunksize_clause
    : ^( CHUNKSIZE LONG ) { sb.append(" ").append($CHUNKSIZE.text).append(" ").append($LONG.text); }
;

rate_clause
    : ^( RATE FLOAT ) { sb.append(" ").append($RATE.text).append(" ").append($FLOAT.text); }
;
...
eid : rel_str_op
    ...
    | CHUNKSIZE { sb.append($CHUNKSIZE.text); }
    | RATE      { sb.append($RATE.text); }
