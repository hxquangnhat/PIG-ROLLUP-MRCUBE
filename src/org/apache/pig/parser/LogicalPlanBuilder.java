...
public class LogicalPlanBuilder {
    ...
    LogicalPlanBuilder(PigContext pigContext, String scope,
            Map<String, String> fileNameMap, IntStream input) {
    ...
    String buildDistinctOp(SourceLocation loc, String alias, String inputAlias,
    ...
    String buildLimitOp(SourceLocation loc, String alias, String inputAlias,
            long limit) throws ParserValidationException {
        LOLimit op = new LOLimit(plan, limit);
        return buildOp(loc, op, alias, inputAlias, null);
    }

    String buildLimitOp(SourceLocation loc, LOLimit op, String alias,
            String inputAlias, LogicalExpressionPlan expr)
            throws ParserValidationException {
    ...
    String buildSampleOp(SourceLocation loc, String alias, String inputAlias,
            double value, SourceLocation valLoc)
            throws ParserValidationException {
        ...
        UserFuncExpression udf = new UserFuncExpression(filterPlan,
    ...
    String buildSampleOp(SourceLocation loc, LOFilter filter, String alias,
            String inputAlias, LogicalExpressionPlan samplePlan,
            LogicalExpression expr) throws ParserValidationException {

        UserFuncExpression udf = new UserFuncExpression(samplePlan,
                new FuncSpec(RANDOM.class.getName()));
        new LessThanExpression(samplePlan, udf, expr);
        return buildFilterOp(loc, filter, alias, inputAlias, samplePlan);
    }

    String buildUnionOp(SourceLocation loc, String alias,
            List<String> inputAliases, boolean onSchema)
            throws ParserValidationException {
    ...
    String buildSplitOutputOp(SourceLocation loc, LOSplitOutput op,
            String alias, String inputAlias, LogicalExpressionPlan filterPlan)
            throws ParserValidationException {
        op.setFilterPlan(filterPlan);
        return buildOp(loc, op, alias, inputAlias, null);
    }

    String buildSplitOtherwiseOp(SourceLocation loc, LOSplitOutput op,
            String alias, String inputAlias) throws ParserValidationException,
        ...
        try {
            ...
            // (PIG-3641)
            new ResetProjectionAttachedRelationalOpVisitor(splitPlan, op)
                    .visit();
        } catch (FrontendException e) {
            e.printStackTrace();
            throw new PlanGenerationFailureException(intStream, loc, e);
    ...
    String buildCrossOp(SourceLocation loc, String alias,
    ...
    String buildSortOp(SourceLocation loc, LOSort sort, String alias,
            String inputAlias, List<LogicalExpressionPlan> plans,
    ...
    String buildRankOp(SourceLocation loc, LORank rank, String alias,
            String inputAlias, List<LogicalExpressionPlan> plans,
    ...
    String buildJoinOp(SourceLocation loc, LOJoin op, String alias,
            List<String> inputAliases,
            MultiMap<Integer, LogicalExpressionPlan> joinPlans, JOINTYPE jt,
            List<Boolean> innerFlags, String partitioner)
            throws ParserValidationException {
        ...
        } else if ((jt == JOINTYPE.MERGE || jt == JOINTYPE.MERGESPARSE)
                && inputCount != 2) {
            throw new ParserValidationException(intStream, loc,
    ...
    /**
     * Set the value of the pivot from user input
     * @param op
     * @param pivot
     * @throws ParserValidationException
     */
    void setPivotRollupCubeOp(LOCube op, Integer pivot)
            throws ParserValidationException {
        if (pivot != null)
            op.setPivot(pivot);
    }
    ...
    /**
     * Set the value of the chunksize from user input
     * @param op
     * @param chunksize
     */
    void setChunkSizeRollupCubeOp(LOCube op, Integer chunksize) {
        if (chunksize != null)
            op.setChunkSize(chunksize);
    }

    /**
     * Set the value of the rate from user input
     * @param op
     * @param rate
     */
    void setRateRollupCubeOp(LOCube op, Double rate) {
        if (rate !=null)
            op.setRate(rate);
    }

    String buildCubeOp(SourceLocation loc, LOCube op, String alias,
            String inputAlias, List<String> operations,
            MultiMap<Integer, LogicalExpressionPlan> expressionPlans)
            throws ParserValidationException {

        // check value of pivot, chunksize, rate if they are valid or not
        try {
            if (op.getPivot() != -1) {
                if (op.getPivot() < 0
                        || op.getPivot() >= expressionPlans.get(0).size()) {
                    FrontendException fe = new FrontendException(
                            "PIVOT is out of bound");
                    throw fe;
                }
            }
            if (op.getChunkSize() != -1) {
                if (op.getChunkSize() <= 0) {
                    FrontendException fe = new FrontendException(
                            "CHUNKSIZE must be larger than 0");
                    throw fe;
                }
            }
            if (op.getRate() != 0) {
        ...
        try {
            alias = convertCubeToFGPlan(loc, op, inputAlias, operations,
                    expressionPlans);
        } catch (FrontendException e) {
            throw new ParserValidationException(intStream, loc, e);
        }
        ...
        for (i = 0; i < operations.size(); i++) {
            if ((startIdx == -1) && (operations.get(i).equals("CUBE") == true)) {
                ...
                if (operations.get(i).equals("CUBE") == true) {
                    ...
                    if (endIdx > startIdx) {
                        mergeAndMarkForDelete(operations, expressionPlans,
                                startIdx, endIdx);
                        isMerged = true;
                        startIdx = -1;
                        endIdx = -1;
    ...
    private String convertCubeToFGPlan(SourceLocation loc, LOCube op,
        ...
        for (Operator oper : inpOpers) {
            ...
            if (schema != null) {
                ...
                for (int i = 0; i < fields.size(); i++) {
                    ...
                    new ProjectExpression(lEplan, i, fields.get(i).alias, null,
                            gen);
        ...
        for (int operIdx = 0; operIdx < operations.size(); operIdx++) {
            ...
            for (int i = 0; i < lexpList.size(); i++) {
                ...
                for (int j = 0; j < allExprPlan.size(); j++) {
                    LogicalExpression lexp = (LogicalExpression) allExprPlan
                            .get(j).getSources().get(0);
                    String colAlias = ((ProjectExpression) lexpList.get(i))
                            .getColAlias();
                    if (colAlias == null) {
                        colAlias = ((ProjectExpression) lexpList.get(i))
                                .getFieldSchema().alias;
                    }
                    ...
                    if (colAlias.equals(projExpAlias) == true) {
                        ...
                        if (projExpAlias.lastIndexOf(":") != -1) {
                            projExpAlias = projExpAlias.substring(projExpAlias
                                    .lastIndexOf(":") + 1, projExpAlias
                                    .length());
                            if (colAlias.equals(projExpAlias) == true) {
                                allExprPlan.remove(j);
                            }
            ...
            if (operations.get(operIdx).equals("CUBE")) {
                new UserFuncExpression(uexpPlan, new FuncSpec(
                        CubeDimensions.class.getName()), lexpList);
            } else {
                new UserFuncExpression(uexpPlan, new FuncSpec(
                        RollupDimensions.class.getName()), lexpList);

                //If no pivot, no chunksize and no rate were specified,
                //default rate and default chunksize will be set.
                if (op.getChunkSize() == -1 && op.getRate() == 0){
                    op.setChunkSize(DEFAULT_CHUNKSIZE);
                    op.setRate(DEFAULT_RATE);
                }
            }
        ...
        try {
            buildGenerateOp(loc, (LOForEach) foreach, (LOGenerate) gen,
                    allExprPlan, flattenFlags,
                    getUserDefinedSchema(allExprPlan));
            falias = buildForeachOp(loc, (LOForEach) foreach, "cube",
                    inputAlias, innerPlan);
        ...
        for (LogicalExpressionPlan exp : expressionPlans.values()) {
            LogicalExpression lexp = (LogicalExpression) exp.getSources()
                    .get(0);
            LogicalExpressionPlan epGrp = new LogicalExpressionPlan();
            new ProjectExpression(epGrp, 0, lexp.getFieldSchema().alias, null,
                    groupby);
        ...
        try {
            //Set value of pivot, chunksize and rate for groupby operator
            groupby.setPivot(op.getPivot());
            groupby.setChunkSize(op.getChunkSize());
            groupby.setRate(op.getRate());
            return buildGroupOp(loc, (LOCogroup) groupby, op.getAlias(),
                    inpAliases, exprPlansCopy, GROUPTYPE.REGULAR, innerFlags,
                    null);
        } catch (ParserValidationException pve) {
            throw new FrontendException(pve);
        }
    ...
    // of UDF will be used which will prefix "dimensions" namespace to all
    // fields
    private List<LogicalSchema> getUserDefinedSchema(
            List<LogicalExpressionPlan> allExprPlan) throws FrontendException {
            ...
            for (Operator oper : opers) {
                ...
                if (oper instanceof ProjectExpression) {
                    ...
                    output.addField(new LogicalFieldSchema(
                            ((ProjectExpression) oper).getColAlias(), null,
                            DataType.NULL));
                    ...
                    LogicalSchema output = new LogicalSchema();
                    for (Operator op : ((UserFuncExpression) oper).getPlan()
                            .getSinks()) {
                        output.addField(new LogicalFieldSchema(
                                ((ProjectExpression) op).getFieldSchema()));
                    }
                    genOutputSchema.add(output);
                }
    ...
    private List<LogicalExpression> getProjectExpList(
            List<LogicalExpressionPlan> lexpPlanList,
        ...
        for (int i = 0; i < lexpPlanList.size(); i++) {
            ...
            LogicalExpression lex = (LogicalExpression) lexp.getSources()
                    .get(0);
    ...
    private void checkDuplicateProject(List<LogicalExpressionPlan> lExprPlan)
        ...
        for (int i = 0; i < lExprPlan.size(); i++) {
            for (int j = i + 1; j < lExprPlan.size(); j++) {
                LogicalExpression outer = (LogicalExpression) lExprPlan.get(i)
                        .getSources().get(0);
                LogicalExpression inner = (LogicalExpression) lExprPlan.get(j)
                        .getSources().get(0);
                ...
                if (outColAlias.equals(inColAlias) == true) {
                    ...
                    throw new FrontendException(
                            "Duplicate dimensions detected. Dimension name: "
                                    + inColAlias);
                }
            }
        }
    ...
    String buildGroupOp(SourceLocation loc, LOCogroup op, String alias,
            List<String> inputAliases,
            MultiMap<Integer, LogicalExpressionPlan> expressionPlans,
            GROUPTYPE gt, List<Boolean> innerFlags, String partitioner)
            throws ParserValidationException {
    ...
    String buildLoadOp(SourceLocation loc, String alias, String filename,
        ...
        try {
            ...
            // Fallback on PigStorage if LoadFunc is not specified in
            // properties.
            funcSpec = funcSpec == null ? new FuncSpec(pigContext
                    .getProperties().getProperty(
                            PigConfiguration.PIG_DEFAULT_LOAD_FUNC,
                            PigStorage.class.getName())) : funcSpec;
            loFunc = (LoadFunc) PigContext.instantiateFuncFromSpec(funcSpec);
            String fileNameKey = QueryParserUtils.constructFileNameSignature(
                    filename, funcSpec)
                    + "_" + (loadIndex++);
        ...
        LOLoad op = new LOLoad(loader, schema, plan, ConfigurationUtil
                .toConfiguration(pigContext.getProperties()), loFunc, alias
                + "_" + newOperatorKey());
        op.setTmpLoad(false);
        return buildOp(loc, op, alias, new ArrayList<String>(), null);
    }

    private String buildOp(SourceLocation loc, LogicalRelationalOperator op,
            String alias, String inputAlias, String partitioner)
            throws ParserValidationException {
    ...
    private void checkDuplicateAliases(List<String> inputAliases,
            SourceLocation loc, String opName) throws ParserValidationException {
    ...
    private String buildOp(SourceLocation loc, LogicalRelationalOperator op,
            String alias, List<String> inputAliases, String partitioner)
            throws ParserValidationException {
        setAlias(op, alias);
    ...
    String buildStoreOp(SourceLocation loc, String alias, String inputAlias,
            String filename, FuncSpec funcSpec)
            throws ParserValidationException {
        try {
            // Load StoreFunc class from default properties if funcSpec is null.
            // Fallback on PigStorage if StoreFunc is not specified in
            // properties.
            funcSpec = funcSpec == null ? new FuncSpec(pigContext
                    .getProperties().getProperty(
                            PigConfiguration.PIG_DEFAULT_STORE_FUNC,
                            PigStorage.class.getName())) : funcSpec;
            StoreFuncInterface stoFunc = (StoreFuncInterface) PigContext
    ...
    String buildAssertOp(SourceLocation loc, LOFilter filterOp, String alias,
            String inputAlias, LogicalExpression expr, String comment,
            LogicalExpressionPlan exprPlan) throws ParserValidationException {
        try {
            filterOp.setAlias(inputAlias);
            List<LogicalExpression> args = new ArrayList<LogicalExpression>();
            ConstantExpression lhs = new ConstantExpression(exprPlan,
                    new Boolean(true));
            ConstantExpression rhs = new ConstantExpression(exprPlan,
                    new Boolean(false));
            BinCondExpression binCond = new BinCondExpression(exprPlan, expr,
                    lhs, rhs);
            args.add(binCond);
            ConstantExpression constExpr = new ConstantExpression(exprPlan,
                    (comment == null ? "" : comment));
            args.add(constExpr);
            UserFuncExpression udf = new UserFuncExpression(exprPlan,
                    new FuncSpec(Assert.class.getName()), args);
            exprPlan.add(udf);
            filterOp.setFilterPlan(exprPlan);
            // pass the inputAlias to alias
            return buildFilterOp(loc, filterOp, inputAlias, inputAlias,
                    exprPlan);
        } catch (Exception ex) {
            throw new ParserValidationException(intStream, loc, ex);
        }
    ...
    String buildForeachOp(SourceLocation loc, LOForEach op, String alias,
    ...
    void buildGenerateOp(SourceLocation loc, LOForEach foreach, LOGenerate gen,
        ...
        int idx = 0;
        for (LogicalExpressionPlan exprPlan : exprPlans) {
            LogicalExpression expr = (LogicalExpression) exprPlan.getSources()
                    .get(0);
    ...
    private static void processExpressionPlan(LOForEach foreach,
            LogicalPlan lp, LogicalExpressionPlan plan,
            ...
            if (sink instanceof ProjectExpression) {
                ...
                if (projExpr.isRangeProject()) {
                    ...
                    LOInnerLoad innerLoad = new LOInnerLoad(lp, foreach,
                            new ProjectExpression(projExpr,
                                    new LogicalExpressionPlan()));
    ...
    Operator buildNestedOperatorInput(SourceLocation loc,
            LogicalPlan innerPlan, LOForEach foreach,
    ...
    private LOInnerLoad createInnerLoad(SourceLocation loc,
            LogicalPlan innerPlan, LOForEach foreach, String colAlias)
            throws ParserValidationException {
        try {
    ...
    StreamingCommand buildCommand(SourceLocation loc, String cmd,
            List<String> shipPaths, List<String> cachePaths,
            List<HandleSpec> inputHandleSpecs,
            List<HandleSpec> outputHandleSpecs, String logDir, Integer limit)
    ...
    StreamingCommand buildCommand(SourceLocation loc, String cmd)
        ...
        try {
            ...
            StreamingCommandUtils validator = new StreamingCommandUtils(
                    pigContext);
    ...
    String buildStreamOp(SourceLocation loc, String alias, String inputAlias,
            StreamingCommand command, LogicalSchema schema, IntStream input)
            throws RecognitionException {
        try {
            LOStream op = new LOStream(plan, pigContext
                    .createExecutableManager(), command, schema);
            return buildOp(loc, op, alias, inputAlias, null);
    ...
    String buildNativeOp(SourceLocation loc, String inputJar, String cmd,
            ...
            IntStream input) throws RecognitionException {
        LONative op;
        try {
            op = new LONative(plan, inputJar, StreamingCommandUtils
                    .splitArgs(cmd));
    ...
    void setParallel(LogicalRelationalOperator op, Integer parallel) {
        if (parallel != null) {
            op.setRequestedParallelism(pigContext.getExecType() == ExecType.LOCAL ? 1
                    : parallel);
        }
    }
    ...
    FuncSpec buildFuncSpec(SourceLocation loc, String funcName,
            List<String> args, byte ft) throws RecognitionException {
        String[] argArray = new String[args.size()];
        FuncSpec funcSpec = new FuncSpec(funcName, args.size() == 0 ? null
                : args.toArray(argArray));
        validateFuncSpec(loc, funcSpec, ft);
        return funcSpec;
    }

    private void validateFuncSpec(SourceLocation loc, FuncSpec funcSpec, byte ft)
            throws RecognitionException {
        switch (ft) {
        ...
        case FunctionType.PIGTOSTREAMFUNC:
            try {
                Class<?> func = PigContext.resolveClassName(funcSpec
                        .getClassName());
    ...
    LogicalExpression buildProjectExpr(SourceLocation loc,
            ...
            Map<String, LogicalExpressionPlan> exprPlans, String colAlias,
            int col) throws RecognitionException {
        ...
        if (colAlias != null) {
            ...
            if (exprPlan != null) {
                ...
                LogicalExpression root = (LogicalExpression) planCopy
                        .getSources().get(0);// get the root of the plan
                ...
                try {
                    ...
                    // Sometimes it can throw an exception. If it does, then
                    // there is no schema to get
                }
                return root;
            } else {
                result = new ProjectExpression(plan, 0, colAlias, operators
                        .get(colAlias), op);
    ...
    LogicalExpression buildProjectExpr(SourceLocation loc,
            ...
            throws ParserValidationException {
        ProjectExpression result = null;
        result = colAlias != null ? new ProjectExpression(plan, input,
                colAlias, null, relOp) : new ProjectExpression(plan, input,
                col, relOp);
        result.setLocation(loc);
        return result;
    }
     ...
     * @throws ParserValidationException
     */
    LogicalExpression buildRangeProjectExpr(SourceLocation loc,
            LogicalExpressionPlan plan, LogicalRelationalOperator relOp,
    ...
    private void checkRangeProjectExpr(SourceLocation loc,
            LogicalExpression startExpr) throws ParserValidationException {
    ...
    LogicalExpression buildInvokerUDF(SourceLocation loc,
            LogicalExpressionPlan plan, String packageName, String funcName,
            boolean isStatic, List<LogicalExpression> args)
            throws RecognitionException {
        LogicalExpression le = new UserFuncExpression(plan, new FuncSpec(
                InvokerGenerator.class.getName()), args, false, true, isStatic,
    ...
    public static Class<?> typeToClass(Class<?> clazz) {
        if (clazz == Integer.TYPE) {
            ...
            throw new RuntimeException("Was not given a primitive TYPE class: "
                    + clazz);
        }
    }
    ...
    Operator buildNestedFilterOp(SourceLocation loc, LOFilter op,
            LogicalPlan plan, String alias, Operator inputOp,
    ...
    Operator buildNestedDistinctOp(SourceLocation loc, LogicalPlan plan,
    ...
    Operator buildNestedLimitOp(SourceLocation loc, LogicalPlan plan,
            String alias, Operator inputOp, long limit) {
        LOLimit op = new LOLimit(plan, limit);
        buildNestedOp(loc, plan, op, alias, inputOp);
        return op;
    }

    Operator buildNestedLimitOp(SourceLocation loc, LOLimit op,
            LogicalPlan plan, String alias, Operator inputOp,
            LogicalExpressionPlan expr) {
        op.setLimitPlan(expr);
        buildNestedOp(loc, plan, op, alias, inputOp);
    ...
    Operator buildNestedCrossOp(SourceLocation loc, LogicalPlan plan,
    ...
    private void buildNestedOp(SourceLocation loc, LogicalPlan plan,
            LogicalRelationalOperator op, String alias, Operator inputOp) {
        op.setLocation(loc);
        setAlias(op, alias);
        plan.add(op);
    ...
    private void buildNestedOp(SourceLocation loc, LogicalPlan plan,
            LogicalRelationalOperator op, String alias,
            List<Operator> inputOpList) {
        op.setLocation(loc);
        setAlias(op, alias);
        plan.add(op);
    ...
    Operator buildNestedSortOp(SourceLocation loc, LOSort op, LogicalPlan plan,
            String alias, Operator inputOp, List<LogicalExpressionPlan> plans,
            List<Boolean> ascFlags, FuncSpec fs) {
    ...
    Operator buildNestedForeachOp(SourceLocation loc, LOForEach op,
            LogicalPlan plan, String alias, Operator inputOp,
            LogicalPlan innerPlan) throws ParserValidationException {
        op.setInnerPlan(innerPlan);
        buildNestedOp(loc, plan, op, alias, inputOp);
        return op;
    }

    Operator buildNestedProjectOp(SourceLocation loc, LogicalPlan innerPlan,
            LOForEach foreach, Map<String, Operator> operators, String alias,
            ProjectExpression projExpr, List<LogicalExpressionPlan> exprPlans)
            throws ParserValidationException {
        Operator input = null;
        String colAlias = projExpr.getColAlias();
        if (colAlias != null) {
        ...
        for (LogicalExpressionPlan plan : exprPlans) {
            ...
            LOInnerLoad iload = (al == null) ? new LOInnerLoad(lp, f, pe
                    .getColNum()) : createInnerLoad(loc, lp, f, al);
    ...
    GROUPTYPE parseGroupType(String hint, SourceLocation loc)
            throws ParserValidationException {
    ...
    JOINTYPE parseJoinType(String hint, SourceLocation loc)
            throws ParserValidationException {
        String modifier = unquote(hint);
    ...
    public String getLastRel(SourceLocation loc)
        ...
        if (lastRel == null) {
            throw new ParserValidationException(intStream, loc,
                    "Asked for last relation -- no relations have been defined");
        }
        return lastRel;
    }
