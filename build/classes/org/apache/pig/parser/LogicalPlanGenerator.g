...
chunksize_clause returns[int chunksize]
 : ^( CHUNKSIZE INTEGER )
   {
       $chunksize = Integer.parseInt( $INTEGER.text );
   }
;

rate_clause returns[double rate]
 : ^( RATE DOUBLENUMBER )
   {
       $rate = Double.parseDouble( $DOUBLENUMBER.text );
   }
;
...
@init {
 ...
 : ^( CUBE { $operation = "CUBE"; } cube_by_expr_list { $plans = $cube_by_expr_list.plans; } ) | ^( ROLLUP { $operation = "ROLLUP"; } cube_by_expr_list { $plans = $cube_by_expr_list.plans; } ( pivot_clause { builder.setPivotRollupCubeOp($cube_clause::cubeOp, $pivot_clause.pivot); } | ( chunksize_clause { builder.setChunkSizeRollupCubeOp($cube_clause::cubeOp, $chunksize_clause.chunksize); } rate_clause { builder.setRateRollupCubeOp($cube_clause::cubeOp, $rate_clause.rate); } ) )? )
;
